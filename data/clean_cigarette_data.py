...
cig_df = pd.read_csv('data/cigarette_data.csv')
...
# cig_df.loc[cig_df['county'].str.endswith(" City"), 'county_short'] = cig_df['county'].str.removesuffix(" City")
cig_df.loc[cig_df['county'].str.endswith(" Municipality"), 'county_short'] = cig_df['county'].str.removesuffix(" Municipality") 

# of note:
# - alaska is WEIRD not consistent, lots of specific suffixes to drop
# - suffix of 'city' is inconsistent
#   - i.e., cigarette data has Harrisonburg City as a county name, hdByCounty has Harrisonburg as the county
#   -       cigarette data and hdByCounty BOTH have 'Richmond City' as a county name
#   -       I opted to drop city, as there werent' many counties_city in hdByCounty, but some data will be lost in the join
...
# cig_df.drop(['cig_sex'], axis=1, inplace=True) # drop sex and year, irrelevant now
...
# print(cig_df.shape[0])
# count = 0
# for row_index in range(cig_df.shape[0]):
#     this_row = cig_df.iloc[row_index]
#     print(this_row)
...
# cig_df['test'] = cig_df.groupby(['county_state'])['cig_year'].transform(lambda x : ''.join(str(x)))
# cig_df_out = cig_df.set_index(['county_state', cig_df.groupby(['county_state']).cumcount()+1]).unstack().sort_index(level=1, axis=1)
...
cig_df.to_csv('data/cleaned_cigarette_data.csv', index = False)